...
def trace_to_end(conversion_maps, start_idx, value):
    ...
    return trace_to_end(conversion_maps, start_idx+1, next_value)

def reverse_trace(conversion_maps, start_idx, value):
    if start_idx == 0:
        return value
    next_value = reverse_lookup(conversion_maps[start_idx-1], value)
    return trace_to_end(conversion_maps, start_idx-1, next_value)

def trace_to_seed(seeds, conversion_maps, start_idx, value):
    value = reverse_trace(conversion_maps, start_idx, value)
    if value in seeds:
        return True
    else:
        return False
...
def part_1(data):
    ...
    while not done:
        lowest, idx = find_lowest(lowest_table)
        if lowest == None:
            break
        original_lowest = lowest
        end = trace_to_end(conversion_maps, idx, lowest)
        if end == original_lowest:
            print("end: ", end, "lowest:", original_lowest)
            print()
            print()
            if trace_to_seed(seeds, conversion_maps, idx, lowest):
                return lowest
